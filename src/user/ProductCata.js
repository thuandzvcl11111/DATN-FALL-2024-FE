...
const ProductCata = () => {
  ...
  const [sm_cate, setSm_cate] = useState([]);
  useEffect(() => {
    fetch("http://localhost:3000/cate_small?cataId=" + id)
      .then((res) => res.json())
      .then((blog) => {
        setSm_cate(blog);
      });
        ...
        <div className="container">
          <div className="flex-w flex-sb-m p-b-52">
            <div className="flex-w flex-l-m filter-tope-group m-tb-10">
              {[tatca, ...sm_cate.map((ct, i) => (
                <button
                  className="stext-106 cl6 hov1 bor3 trans-04 m-r-32 m-tb-5 how-active1"
                  data-filter=".balo"
                >
                  <Link to={"/pdt/danhmuc/danhmucnho/" + ct.id}>{ct.name}</Link>
                </button>
              ))]}
            </div>
